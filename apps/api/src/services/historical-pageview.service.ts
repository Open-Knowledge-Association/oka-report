  ...
  async syncYear(year: number, jobId: string) {
    ...
    for (const article of articles.slice(processed)) {
      try {
        ...
        const items = await this.wikimedia.getPageviews(
          article.title,
          project,
          start,
          end,
        ...
        for (const item of items) {
          ...
          await this.prisma.historicalPageview.upsert({
            where: {
              articleId_periodStart_granularity_agentType: {
      ...
      if (processed % 10 === 0 || processed === articles.length)
        await checkpoint(`Historical pageviews ${year} (${processed}/${articles.length})`);
    }
