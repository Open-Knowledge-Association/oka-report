...
async function fetchBatch(project: string, articles: Article[]) {
  ...
  for (let attempt = 0; attempt < 7; attempt++) {
    ...
    if (response.ok) {
      const body = await response.json() as { query?: { pages?: Array<{ title: string; pageid?: number; missing?: boolean }> } };
      return new Map((body.query?.pages ?? [])
...
async function main() {
  const articles = await prisma.article.findMany({
    ...
    select: { id: true, title: true, wikiProject: true }, orderBy: { id: "asc" },
  });
  let updated = 0, missing = 0, errors = 0;
  const groups = new Map<string, Article[]>();
  for (const article of articles) {
    const group = groups.get(article.wikiProject) ?? [];
    group.push(article); groups.set(article.wikiProject, group);
  }
    ...
    for (let offset = 0; offset < projectArticles.length; offset += 50) {
      ...
      try {
        ...
        for (const article of batch) {
          ...
          if (pageId == null) { missing++; continue; }
          try { await prisma.article.update({ where: { id: article.id }, data: { pageId } }); updated++; }
          catch (error) { errors++; console.error(`[pageid] conflict ${article.title}: ${error instanceof Error ? error.message : String(error)}`); }
        }
      } catch (error) {
        errors += batch.length;
        console.error(`[pageid] batch ${project} ${offset}/${projectArticles.length}: ${error instanceof Error ? error.message : String(error)}`);
      }
      console.log(`[pageid] ${Math.min(offset + 50, projectArticles.length)}/${projectArticles.length} project=${project} updated=${updated} missing=${missing} errors=${errors}`);
      await sleep(1000);
    }
  }
